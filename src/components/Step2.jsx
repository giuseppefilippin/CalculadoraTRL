...
function Step2({ formData, onFinish, initialAnswers }) {
  ...
  const salvarResultadoNoFirebase = async (notaFinal) => {
    ...
    const doc = {
      ...formData,
      ...formDataSafe,
      nomeTecnologia: formData.nomeTecnologia || "Projeto sem nome",
      produto: formData.produto || "Sem descrição",
      notaFinal,
      respostasPorNivel,
      userId: user.uid,
          ...
          <div className="mt-8 p-4 bg-gray-50 border border-gray-200 rounded-lg">
            <div className="flex items-center gap-2 mb-3">
              ...
              <h4 className="font-semibold text-gray-800">
                Comentários Gerais sobre TRL {trlAtualNumero}
              </h4>
            </div>
            <p className="text-sm text-gray-600 mb-3">
              Use este espaço para adicionar observações, contexto adicional ou
              considerações específicas sobre este nível TRL em relação ao seu
              projeto.
            </p>
            <textarea
              rows={4}
              ...
              onChange={(e) =>
                handleTrlCommentChange(currentTrlIndex, e.target.value)
              }
